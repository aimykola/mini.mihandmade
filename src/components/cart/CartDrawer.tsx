'use client'
  ...
  const orderText =
    'Вітаю! Хочу замовити:%0A' +
            items.map((i) => `- ${i.product.name} x${i.qty} (${i.product.price} грн)`).join('%0A') +
        `%0AРазом: ${total} грн`
  ...
  return (
    ...
    {sent ? (
                  <div className="rounded-xl bg-brand-soft/20 p-4 text-center">
                    ...
                    <p className="mt-1 text-sm text-foreground/70">Надішліть замовлення нам у Direct, і ми звʼяжемось з вами.</p>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-2">
                    ...
                    <button type="submit" className="w-full rounded-lg bg-brand px-4 py-3 font-semibold text-white transition hover:bg-brand-dark">Оформити замовлення</button>
