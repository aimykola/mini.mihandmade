            ...
            <article key={p.id} className="flex flex-col overflow-hidden rounded-2xl border border-black/10 bg-white shadow-sm transition hover:shadow-md">
              ...
              <div className="flex flex-1 flex-col p-4">
                ...
                <div className="mt-4 flex items-center justify-between">
                                    {(p.discount ?? 0) > 0 ? (<span className="flex items-baseline gap-2"><span className="text-sm text-foreground/40 line-through">{p.price} грн</span><span className="text-xl font-extrabold text-[#b5552e]">{Math.round(p.price * (1 - (p.discount ?? 0) / 100))} грн</span></span>) : (<span className="text-xl font-extrabold text-brand-dark">{p.price} грн</span>)}
